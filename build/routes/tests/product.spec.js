...
describe('Product Handler', function () {
    ...
    it('gets the index endpoint', function (done) {
        request.get('/api/products').then(function (res) {
            expect(res.status).toBe(200);
            done();
        });
    });
    it('gets the read endpoint', function (done) {
        request.get("/api/products/".concat(productId)).then(function (res) {
            expect(res.status).toBe(200);
            done();
        });
    });
    it('gets the update endpoint', function (done) {
        var newProductData = __assign(__assign({}, product), { name: 'CodeMerge 156 A', price: 1299 });
        request
            .patch("/api/products/".concat(productId))
            .send(newProductData)
            .then(function (res) {
            expect(res.status).toBe(200);
            done();
    ...
    it('gets the delete endpoint', function (done) {
        request.delete("/api/products/".concat(productId)).then(function (res) {
            expect(res.status).toBe(200);
            done();
        });
    });
